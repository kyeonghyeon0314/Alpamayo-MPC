...
출력 스키마 ({clip_id}.h5):
  ...
  추론 데이터 (매 step_us마다):
    ...
    v0                (N,)        float32 — 각 프레임 초기 속도 [m/s]
    cotend            (N, 4096)   float32 — R1 VLM cotend hidden state
    gt_xyz_local      (N, 64, 3)  float32 — GT 미래 궤적 (로컬 프레임, x=전방)
    gt_yaw_local      (N, 64)     float32 — GT 미래 yaw (로컬 프레임)
    global_origin_xy  (N, 2)      float64 — 전역 좌표 원점 [m] (전체 주행 궤적)
    global_yaw        (N,)        float64 — 전역 헤딩 [rad]
...
실행 예시 (Docker 컨테이너 내부):
  python /workspace/closed-loop-simul/build_pcl_dataset.py \\
      --manifest  /workspace/closed-loop-simul/split_manifest.json \\
...
def _infer_one(model, processor, raw: dict, device: torch.device) -> dict | None:
    ...
    Returns:
        {cotend, gt_xyz, gt_yaw, v0, front_frame}
        추론 실패 시 None 반환.
    """
    from alpamayo_r1 import helper
    from alpamayo_r1.dataset.schema import derive_ego_states

    try:
        # ── 모델 입력 구성 ────────────────────────────────────
        # flatten: (N_cam, N_frm, C, H, W) → (N_cam*N_frm, C, H, W)
        messages  = helper.create_message(raw["image_frames"].flatten(0, 1))
        ...
        # ── 초기 속도 v0 (히스토리 마지막 스텝 속도) ─────────
        ego_hist_xyz = raw["ego_history_xyz"].cpu().numpy()[0, 0]  # (16, 3)
        ego_hist_rot = raw["ego_history_rot"].cpu().numpy()[0, 0]  # (16, 3, 3)
        ego_hist_ego = derive_ego_states(ego_hist_xyz, ego_hist_rot)  # (16, 5)
        v0 = float(ego_hist_ego[-1, 2])   # speed 열 (index 2)
...
def _save_clip_h5(
    ...
    with h5py.File(out_path, "w") as f:
        ...
        # ── 추론 데이터 ──────────────────────────────────────
        f.create_dataset("v0",     data=np.array([d["v0"]     for _, d in frames], dtype=np.float32), compression="gzip")
...
def _save_clip_viz(
    ...
    v0_arr   = np.array([d["v0"] for _, d in frames_s], dtype=np.float32)  # [m/s]
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="R1 모델로 PCL 연속 데이터셋 생성 (test 클립 전용)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    p.add_argument(
        "--manifest", required=True, metavar="JSON",
    ...
    p.add_argument(
        ...
        metavar="DIR",
        help="AlpamayoR1 가중치 디렉토리 (기본: alpamayo_weights)",
    )
    p.add_argument(
        "--dataset-dir",
...
def main() -> None:
    ...
    # ── split_manifest.json → test 클립 목록 ─────────────────
    ...
    log.info("대상 클립: test 셋에서 %d개 선택 (전체 %d개)",
             len(test_clips), manifest["n_clips_test"])
