...
3. model.sample_trajectories_from_data_with_vlm_rollout(
       return_cotend_hidden_state=True   ← alpamayo_r1.py 내부 hook이 활성화됨
   )
...
class AlpamayoDatasetCollector:
    ...
    Args:
        model:           로드된 AlpamayoR1 모델 (CUDA 배치 완료 상태)
    ...
    def __init__(
        ...
        *,
        ...
        self._infer_kwargs = dict(
            ...
            return_extra=True,
            return_cotend_hidden_state=True,  # alpamayo_r1.py 내부 hook으로 캡처
        )
    ...
    def _build_sample(
        ...
        t0_us = int(raw["t0_us"])
        history_ts = np.array(
            [t0_us + (-15 + i) * _DT_US for i in range(16)], dtype=np.int64
        )
